...
export class PlayerComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  processConfig(): void {
    ...
    this.subscriptionFolderPath =
      this.postsService.config["Subscriptions"]["subscriptions_base_path"];
    ...
    this.postsService.getFile(this.uid, this.uuid).subscribe((res) => {
      ...
      if (!this.db_file) {
        this.postsService.openSnackBar(
          $localize`Failed to get file information from the server.`,
          "Dismiss"
        );
        return;
      }
      this.postsService
        .incrementViewCount(this.db_file["uid"], null, this.uuid)
        .subscribe(
          () => undefined,
          (err) => {
      ...
      this.type = this.db_file["isAudio"]
        ? ("audio" as FileType)
        : ("video" as FileType);
      this.parseFileNames();
    });
  ...
  getSubscription(): void {
    this.postsService.getSubscription(this.sub_id).subscribe(
      (res) => {
      ...
      () => {
        ...
        this.postsService.openSnackBar(
          `Failed to find subscription ${this.sub_id}`,
          "Dismiss"
        );
      }
    );
  ...
  getPlaylistFiles(): void {
    this.postsService.getPlaylist(this.playlist_id, this.uuid, true).subscribe(
      (res) => {
        if (res["playlist"]) {
  ...
  downloadContent(): void {
    ...
    this.postsService
      .downloadPlaylistFromServer(this.playlist_id, this.uuid)
      .subscribe(
        (res) => {
  ...
  downloadFile(): void {
    ...
    this.downloading = true;
    this.postsService.downloadFileFromServer(this.uid, this.uuid).subscribe(
      (res) => {
        this.downloading = false;
        const blob: Blob = res;
  ...
  openShareDialog(): void {
    const dialogRef = this.dialog.open(ShareMediaDialogComponent, {
      data: {
        ...
        sharing_enabled: this.playlist_id
          ? this.db_playlist.sharingEnabled
          : this.db_file.sharingEnabled,
        is_playlist: !!this.playlist_id,
        uuid: this.postsService.isLoggedIn
          ? this.postsService.user.uid
          : this.uuid,
