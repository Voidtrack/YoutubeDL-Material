...
export class ArchiveViewerComponent {
  ...
  subUploadFilterSelectionChanged(value): void {
    ...
    if (this.upload_sub_id !== "none") {
      this.upload_type = this.postsService.getSubscriptionByID(
        this.upload_sub_id
      )["type"];
    }
  }
  ...
  getArchives(): void {
    this.postsService
      .getArchives(
        this.type === "both" ? null : this.type,
        this.sub_id === "none" ? null : this.sub_id
      )
      .subscribe((res) => {
        if (
          res["archives"] !== null &&
    ...
    for (const droppedFile of this.files) {
      ...
      if (droppedFile.fileEntry.isFile) {
        ...
        fileEntry.file(async (file: File) => {
          ...
          this.postsService
            .importArchive(
              archive_base64 as string,
              this.upload_type,
              this.upload_sub_id === "none" ? null : this.upload_sub_id
            )
            .subscribe(
              (res) => {
                ...
                if (res["success"]) {
                  ...
                  this.postsService.openSnackBar(
                    $localize`Archive successfully imported!`
                  );
                }
                this.getArchives();
              },
  ...
  deleteSelectedArchives(): void {
    for (const archive of this.selection.selected) {
      this.archives = this.archives.filter(
        (_archive: Archive) =>
          !(
            archive["extractor"] === _archive["extractor"] &&
            archive["id"] !== _archive["id"]
          )
      );
    }
    this.postsService
      .deleteArchiveItems(this.selection.selected)
      .subscribe((res) => {
        if (res["success"]) {
          this.postsService.openSnackBar(
            $localize`Successfully deleted archive items!`
          );
        } else {
          this.postsService.openSnackBar(
            $localize`Failed to delete archive items!`
          );
        }
        this.getArchives();
      });
